...
export const normalizeModifiers = (
  modifiers: Modifier[],
): NormalizedModifier[] => [...new Set(modifiers.map(normalizeModifier))];
...
// `Modifier` type guard
export const isModifier = (str: string): str is Modifier =>
  [
    'Command',
    'Cmd',
    'Control',
...
export const normalizeModifier = (modifier: Modifier): NormalizedModifier => {
  switch (modifier) {
    case 'Cmd':
    case 'Command':
      return 'Cmd';
